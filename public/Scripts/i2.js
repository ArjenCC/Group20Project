...
parent.getElementsByClassName("form-control")[0].removeAttribute("readonly");

var b=e.getAttribute("readonly");
console.log(b);
   ...
   else if(e.getAttribute("readonly")){

    
parent.getElementsByClassName("form-control")[0].setAttribute("readonly","readonly");
//console.log(parent);
...
function canEdit(e)
{
    e.parentNode.getElementsByClassName("canEdit")[0].style.visibility="visible";
    
    e.parentNode.getElementsByClassName("canEdit")[1].style.visibility="visible";
}
...
function cantEdit(e)
{
    e.parentNode.getElementsByClassName("canEdit")[0].style.visibility="hidden";
    
    e.parentNode.getElementsByClassName("canEdit")[1].style.visibility="hidden";
}
...
function triggertf()
{
    ...
    cnttf=parseInt(document.getElementById("ijsQuestionlength").value);
...
const bfNode=document.getElementById("mcSubmit");
document.getElementById("form").insertBefore(cln, bfNode);
...
//attach node and set name
//alert(cnt);
 cln.children[3].setAttribute("name",`Question`+`${cnt+1}`);
 
 cln.children[3].setAttribute("id",`Question`+`${cnt+1}`);
 cln.children[3].setAttribute("placeholder",`Question `+`${cnt+1}`);
//give label name
cln.children[2].innerHTML=`Question `+`${cnt+1}`;
 //Alsoset proper name for first answer
